...
class Blacklist extends React.Component {
  ...
  handleSubmit(e) {
    e.preventDefault();
    const form = document.forms.addBlacklistTraveller;
    const passenger = {
      name: form.travellername.value,
    };
    console.log(passenger);
  ...
  render() {
    return (
      <form name='addBlacklistTraveller' onSubmit={this.handleSubmit}>
        <input type='text' name='travellername' placeholder='Name' />
...
class TicketToRide extends React.Component {
  ...
  componentDidMount() {
    console.debug("[INFO] Loading data from server...");
    this.loadData();
  }
  ...
  async loadData() {
    ...
    const { listTravellers } = await graphQLFetch(
      `{ listTravellers, { id name phone bookingTime }  }`
    );

    this.setState({ ...this.state, travellers: listTravellers });
    /*End of Q3*/
  }
  ...
  async bookTraveller(passenger) {
     ...
     * - Post process data and take some action (e.g., re-load UI)  */
    const { addTraveller } = await graphQLFetch(
      `mutation  {
        addTraveller(
          ticket: {  
            name: "${passenger.name}", 
  ...
  render() {
    return (
      <div>
        ...
        {this.state.selector === 5 ? (
          <Blacklist blacklistTraveller={this.blacklistTraveller} />
        ) : (
          <hr />
        )}
      </div>
